...
-- Fonction pour obtenir l'état de l'imprimante
local function getPrinterStatus()
    local paperLevel = printer.getPaperLevel()
    local inkLevel = printer.getInkLevel()

    local paperStatus = "Papier: " .. paperLevel
    local inkStatus = "Encre: " .. inkLevel
    return paperStatus, inkStatus
end
...
local function drawUI()
    ...
    local paperStatus, inkStatus = getPrinterStatus()
    print(paperStatus)
    print(inkStatus)
...
local function printToBook(content)
    ...
    if paperLevel <= 0 or inkLevel <= 0 then
        print("Erreur : Imprimante sans papier ou encre.")
        return
    end
